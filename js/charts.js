...
// Read CSS design tokens at render time — avoids hardcoded colours
...
function buildTCOChart() {
  ...
  tcoChart = new Chart(ctx, {
    ...
    data: {
      ...
      datasets: [{
        ...
        backgroundColor: rows.map(r =>
          r.t.annualCost < 30000 ? clr('--color-success') + '99' :
          r.t.annualCost < 36000 ? clr('--color-warning') + '99' :
                                   clr('--color-danger')  + '99'
        ),
        borderRadius: 4,
        borderSkipped: false
    ...
    options: {
      ...
      scales: {
        x: { ticks: { color: clr('--text-3'), font: { size: 10, family: 'Inter' } }, grid: { display: false }, border: { display: false } },
        y: { ticks: { color: clr('--text-3'), font: { size: 10, family: 'Inter' }, callback: v => '$' + Math.round(v / 1000) + 'k' }, grid: { color: clr('--border') }, border: { display: false } }
      }
    }
  });
...
function buildCOE() {
  ...
  coeChart = new Chart(ctx, {
      ...
      datasets: [
        {
          ...
          backgroundColor: clr('--color-primary-bg'),
          tension: 0.35,
          pointRadius: 4,
        ...
        },
        {
          ...
          borderDash: [5, 4],
          backgroundColor: clr('--color-warning-bg'),
          tension: 0.35,
          pointRadius: 3,
          fill: true,
          borderWidth: 1.5
    ...
    options: {
      responsive: true,
      maintainAspectRatio: false,
      plugins: { legend: { labels: { color: clr('--text-3'), font: { size: 10, family: 'Inter' } } } },
      scales: {
        x: { ticks: { color: clr('--text-3'), font: { size: 9, family: 'Inter' } }, grid: { color: clr('--border') }, border: { display: false } },
        y: { ticks: { color: clr('--text-3'), font: { size: 9, family: 'Inter' }, callback: v => '$' + Math.round(v / 1000) + 'k' }, grid: { color: clr('--border') }, border: { display: false } }
      }
    }
  });
}
